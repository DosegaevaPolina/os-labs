  ...
  while (true) {
    ...
    case client_query_type::send_msg: {
      ...
      if (group_chats.find(recipient_login) == group_chats.end()) {
        if (clients.find(recipient_login) == clients.end()) {
          std::cout << "Client with login '" << recipient_login
                    << "' doesn't exist" << std::endl;
          break;
        }
        int fd_other_client =
            open(("./chats/server_" + recipient_login + ".chat").c_str(), O_RDWR, S_IRUSR | S_IWUSR);
        if (fd_other_client == -1) {
          perror("open failed");
          exit(1);
        }
        ftruncate(fd, PAGE_SIZE);
        char *buff_to_other_client =
            static_cast<char *>(mmap(nullptr, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd_other_client, 0));
        ...
        for (const auto &curr_recipient_login : group_chats[recipient_login]) {
          int fd_other_client =
              open(("./chats/server_" + curr_recipient_login + ".chat").c_str(), O_RDWR, S_IRUSR | S_IWUSR);
          if (fd_other_client == -1) {
            perror("open failed");
            exit(1);
          }
          ftruncate(fd, PAGE_SIZE);
          char *buff_to_other_client = static_cast<char *>(
              mmap(nullptr, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd_other_client, 0)
          );
          while (buff_to_other_client[0] != '0')
            ;
    ...
    case client_query_type::create_group: {
      ...
      if (group_chats.find(group_name) == group_chats.end()) {
        ...
        std::cout << "Group with the name '" << group_name
                  << "' already exists! Please try again" << std::endl;
      }
    ...
    case client_query_type::join_group: {
      ...
      if (group_chats.find(group_name) != group_chats.end()) {
        ...
        std::cout << "Group with the name '" << group_name
                  << "' doesn't exist! Please try again." << std::endl;
      }
...
void *handle_registration(void *args) {
  ...
  char *from_clients = static_cast<char *>(
  ...
  while (true) {
    ...
    std::cout << "Client with login '" << sender_login
              << "' has jumped into the server!" << std::endl;
